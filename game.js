...
const victorianGivenNames=[...maleVictorianNames,...femaleVictorianNames];
...
function resolveEvent(e,i){const c=e.choices[i],before={funds:state.funds,reputation:state.reputation,harmony:state.harmony,loyalty:state.loyalty},month=state.month;Object.entries(c.effects).forEach(([k,v])=>state[k]=(state[k]||0)+v);const monthlyIncome=state.income/12,staffWages=state.staff.reduce((a,s)=>a+s.wage,0),upkeep=65,investmentNotes=[];state.funds+=monthlyIncome-staffWages-upkeep;if(state.investment&&Math.random()<.35){const change=Math.round(state.investment*(Math.random()<.55?.35:-.45)),returned=state.investment+change,note=`The railway shares were sold for ${money(returned)}.`;state.funds+=returned;state.history.unshift(note);investmentNotes.push(note);state.investment=0}investmentNotes.push(...settleHoldings());state.history.unshift(c.result);state.month++;clamp();finishAfterSummary=state.month>=12||state.funds<-500||state.reputation<=0;if(!finishAfterSummary){saveState();renderGame();renderHistory()}showMonthSummary(month,c.result,before,{monthlyIncome,staffWages,upkeep,investmentNotes})}
function settleHoldings(){const notes=[];state.holdings=state.holdings||[];state.holdings=state.holdings.filter(h=>{if(state.month-h.month<2||Math.random()>.34)return true;const type=investmentTypes.find(x=>x.id===h.id);const failed=Math.random()<type.risk;const change=Math.round(h.stake*(failed?-type.downside:type.upside)),returned=h.stake+change,note=`${type.name} ${failed?'disappointed':'rewarded'} the family, returning ${money(returned)}.`;state.funds+=returned;state.reputation+=failed?type.reputation-1:type.reputation;state.history.unshift(note);notes.push(note);return false});return notes}
function signed(value){const rounded=Math.round(value);return `${rounded>0?'+':''}${rounded}`}
function showMonthSummary(month,result,before,account){const months=['January','February','March','April','May','June','July','August','September','October','November','December'],net=state.funds-before.funds,investmentText=account.investmentNotes.length?`<ul>${account.investmentNotes.map(x=>`<li>${x}</li>`).join('')}</ul>`:'<p>No investment matured this month. Existing holdings remain exposed to future gain or loss.</p>';$('#month-summary-content').innerHTML=`<p class="eyebrow">The household account</p><h2 id="month-summary-title">${months[month]} concluded</h2><p>${result}</p><div class="month-account"><div><span>Household income</span><strong>${money(account.monthlyIncome)}</strong></div><div><span>Staff wages</span><strong>−£${account.staffWages}</strong></div><div><span>Household upkeep</span><strong>−£${account.upkeep}</strong></div><div class="total"><span>Overall change in ready funds</span><strong>${net>=0?'+':''}${money(net)}</strong></div></div><h3>Investments</h3>${investmentText}<div class="summary-changes"><div><span>Reputation</span><strong>${signed(state.reputation-before.reputation)}</strong></div><div><span>Family accord</span><strong>${signed(state.harmony-before.harmony)}</strong></div><div><span>Staff confidence</span><strong>${signed(state.loyalty-before.loyalty)}</strong></div></div>`;$('#continue-month').textContent=finishAfterSummary?'Read the family legacy':'Continue to the next month';$('#month-summary').showModal()}
